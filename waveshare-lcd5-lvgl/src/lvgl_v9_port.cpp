...
static void display_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    if (s_use_direct_mode) {
        if (lv_display_flush_is_last(disp)) {
            /* VSYNCs fire continuously (~24Hz) even when LVGL isn't
             * flushing — e.g. while the LVGL task is delayed behind a
             * blocking HTTP poll in loop(). Any of those leaves this task's
             * notification "pending" (a tri-state, not a counter), so a
             * later ulTaskNotifyTake() can return immediately on a STALE
             * vsync that has nothing to do with the switch we're about to
             * request. ulTaskNotifyValueClear() only zeroes the numeric
             * value, not that pending state — doesn't help. Drain any
             * stale pending notification with a non-blocking take FIRST,
             * so the real wait below can only be satisfied by a vsync that
             * happens after switchFrameBufferTo() — i.e. the one that
             * actually confirms this switch took effect. Without this, LVGL
             * could start drawing into the still-visible buffer, which is
             * what the intermittent tearing looked like. */
